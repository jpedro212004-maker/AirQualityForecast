...
def load_data():
    ...
    try:
        ...
        # Padronizar nomes das colunas e datas
    ...
    try:
        ...
        # Limpeza básica similar ao notebook
        df23.columns = df23.columns.str.strip()
        df23 = df23.rename(columns={'Data-Hora': 'Data'})
        df23['Data'] = pd.to_datetime(df23['Data'], errors='coerce')
        df23['Ano'] = 2023
        data["ar_2023"] = df23
    except Exception as e:
        st.warning(f"Aviso: Dados de 2023 não encontrados ({e})")
...
if df2025 is not None:
    ...
    st.error("Não foi possível carregar os dados de 2025.")
    st.stop()
...
if df_meteo is not None:
    # Ajuste de nomes para corresponder (strip e title case)
...
with tab1:
    ...
    # Métricas
    avg_val = df2025_filtrado[poluente].mean()
    max_val = df2025_filtrado[poluente].max()
...
with tab2:
    ...
    if not df2023_filtrado.empty:
        # Calcular médias anuais para o gráfico
        media23 = df2023_filtrado[cols_existentes].mean().reset_index()
        media23.columns = ['Poluente', 'Valor']
        media23['Ano'] = '2023'
        
        media25 = df2025_filtrado[cols_existentes].mean().reset_index()
        ...
        st.info("Dados de 2023 não disponíveis para comparação.")
...
with tab3:
    ...
    if not df_meteo_filtrado.empty:
        ...
        # Mapa de Calor Correlação (Meteo vs Poluentes) - Simplificado
        ...
        df_corr = pd.concat([df_meteo_dia[['temperature_2m', 'rain', 'wind_speed_80m']], df_ar_dia], axis=1).corr()
        
        fig_corr = px.imshow(df_corr, text_auto=True, color_continuous_scale='RdBu_r', title="Matriz de Correlação")
        st.plotly_chart(fig_corr, use_container_width=True)
        
    else:
        st.info("Dados meteorológicos indisponíveis.")
...
with tab4:
    ...
    **Objetivo:** Prever a concentração de poluentes com base na meteorologia e histórico.
    ...
    # Podes adicionar aqui os resultados que descobriste no teu notebook!
    # Exemplo estático baseado na tua análise anterior:
    
    st.subheader("🏆 Melhores Modelos por Poluente")
    
    resultados = {
        "Poluente": ["NO2", "O3", "PM10", "PM2.5", "SO2"],
        "Melhor Modelo": ["MLP (Rede Neural)", "LightGBM", "RandomForest", "MLP", "LightGBM"],
        "MAE (Erro Médio)": ["13.42", "18.29", "3.11", "6.47", "0.43"],
        "R² Score": ["0.30", "-0.44", "0.31", "-0.23", "0.64"]
    }
    st.table(pd.DataFrame(resultados))
    
    st.info("""
    **Conclusões:**
    * O **SO2** foi o poluente mais fácil de prever (R² 0.64).
    * O **O3 (Ozono)** e **PM2.5** mostraram-se difíceis de modelar apenas com dados meteorológicos (R² negativo), indicando a necessidade de mais variáveis (ex: tráfego automóvel).
    * As **Redes Neurais (MLP)** tiveram bom desempenho no NO2.
    """)