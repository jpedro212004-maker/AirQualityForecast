...
st.title("Análise de Qualidade do Ar (Réplica do Notebook)")

# --- 1. FUNÇÕES DE PROCESSAMENTO (Idênticas ao teu Notebook) ---

def tratar_dataset_como_notebook(df):
    """
    Replica a lógica exata do notebook:
    1. Limpa nomes de colunas.
    2. Converte datas.
    3. Converte poluentes para numérico.
    4. PREENCHE NAN COM MÉDIA SEMANAL (Crucial para os valores baterem certo).
    """
    # Limpeza de nomes
    ...
    # Criar colunas auxiliares para o groupby
    ...
    # --- A LÓGICA DE IMPUTAÇÃO DO TEU NOTEBOOK ---
...
def load_data():
    ...
    try:
        ...
        df25 = tratar_dataset_como_notebook(df25)
        data['ar_2025'] = df25
    except Exception as e:
        st.error(f"Erro ao ler QualidadeAr2.xlsx: {e}")
        
    # 2023
    try:
        df23 = pd.read_excel("Qualar2023.xlsx")
        df23 = tratar_dataset_como_notebook(df23)
        data['ar_2023'] = df23
    except Exception as e:
        st.error(f"Erro ao ler Qualar2023.xlsx: {e}")
    ...
    try:
        ...
        pass # Meteo é opcional para os graficos de comparação
...
# --- 3. FILTROS E CÁLCULO DE CLASSES ---
...
# Preparar DF 2023 (df23_periodo)
# IMPORTANTE: Filtrar datas de 2023 para bater com as datas existentes em 2025
df23_clean = pd.DataFrame()
if df23_full is not None:
    df23_temp = df23_full[df23_full['Distrito'].isin(distritos_desejados)].copy()
    
    # Lógica de filtro de data do notebook
    if not df_ar.empty:
...
# ==============================================================================
# VISUALIZAÇÕES (Sequencial como no Notebook)
# ==============================================================================

# 1. BOXPLOTS 2025
st.markdown("### Boxplot de Poluentes em 2025")
...
# 2. COMPARAÇÃO 2023 vs 2025 (Boxplots)
if not df23_clean.empty:
    st.markdown("### Distribuição dos poluentes — comparação 2023 vs 2025")
    
    # Preparar concatenação
    ...
    st.markdown("### Variação Percentual dos Poluentes por Distrito")
    ...
    st.markdown("### Média geral da classe por ano")
    ...
    plt.ylim(0, max(media_geral_23, media_geral_25) * 1.15)
    st.pyplot(fig4)

# 5. CORRELAÇÕES
if df_meteo is not None:
    st.markdown("### Correlações (Meteo vs Qualidade Ar)")
    ...
    # Importante: dropna() para garantir calculo correto
    ...
    st.markdown("### Correlação Meteo vs Poluentes")
...
# ==============================================================================
# MACHINE LEARNING (Lisboa)
# ==============================================================================

st.markdown("### Machine Learning: SVR Autoregressivo (Lisboa)")
...
if len(df_lisboa) > 10:
    ...
    if not df_lisboa.empty:
        ...
        st.warning("Dados insuficientes para Lisboa após criação de lags.")
else:
